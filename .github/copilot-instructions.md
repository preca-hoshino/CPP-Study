...
3. **编写代码**：`P5709[AI].cpp`，添加 AGENT CODE 标注，套用标准模板
4. **测试验证**：编译 → 样例测试(≥1) → 边界测试(≥2，占比≥50%) → 特殊测试(可选)，**确保全部AC**
5. **更新文档** ⚠️：在 `## 📖 题目分类` 添加题号（**去掉[AI]**），如 `P1035, P1424, P1425`
6. **Git提交** ⚠️：`git add "P1425[AI].cpp" && git commit -m "[Add] P1425[AI]"`（必须包含代码+文档）
...
```bash
git status                           # 查看状态
git add "P1234[AI].cpp"                            # 添加所有更改
git commit -m "[Add] P1234[AI]"     # 提交
