...
2. **字符串**: `ch-'0'`(字符→数字) | `'0'+digit`(数字→字符) | `+=`拼接 | `reverse()`反转 | `erase()`删除 | `insert()`插入
...
7. **数字处理**: `num%10`提取个位 | `num/=10`去掉个位 | 字符串反转处理
8. **数据结构**: `stack<T>`栈(LIFO) | `set<T>`集合(去重+查找) | `vector<T>`动态数组
9. **枚举技巧**: 双重循环`i<j`避免重复 | `set`去重结果 | `count()`快速判断存在性
...
**基础计算**: B2001, B2019, B2026, B2027, P1001, P1421, P1422, P1425, P1909
**条件判断**: P1046, P1085, P9686
**数组排序**: P1888
**循环累加**: P1035, P1424, P1089
**格式输出**: B2024
**字符串处理**: B2023, P1307, P1427
**数组标记**: P1047
**集合枚举**: P2141
**图形输出**:
**高精度**: 
